...
import { deleteGalleryAction } from "@/server/actions/gallery";
import { Button } from "@/components/ui/button";

// Delete behind a native <dialog> confirm (modal, keyboard accessible), matching
// the news delete button. Optionally redirects after success (from the detail page).
export function DeleteGalleryButton({
  ...
  function handleConfirm() {
    startTransition(async () => {
      ...
      toast.success("Galerie byla smazána.");
  ...
  return (
    <>
      ...
      >
        <h2 id="delete-gallery-title" className="text-lg font-semibold">
          Smazat galerii
        </h2>
        <p className="text-muted-foreground mt-2 text-sm">
          Opravdu chcete smazat „{name}“? Tuto akci nelze vzít zpět — smažou se
          úvodní obrázek i všechny fotky.
        </p>
